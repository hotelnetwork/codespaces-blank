...
class StockAnalyzer:
    ...
    def plot_indicators(self, indicators):
        ...
        fig.write_html(f'tech/{self.ticker}.php')
...
# Create a PHP file with a Bootstrap form to select the ticker pages
with open('tech/index.php', 'w') as f:
    f.write('<?php\n')
    f.write('?>\n')
    ...
    for ticker in tickers:
        f.write(f'          <option value="{ticker}.php">{ticker}</option>\n')
    f.write('        </select>\n')
    f.write('      </div>\n')
    f.write('    </form>\n')
    f.write('  </div>\n')
